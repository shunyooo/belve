...
一時的なノート・検討メモ (`docs/notes/` — ファイル名は `YYYY-MM-DD-topic.md`):
- 設計判断の記録、移行メモ、パフォーマンス議論、過去仕様のアーカイブなど
...
- **「優しい fallback」を入れない**: 主経路 (新 API / 新パス) が失敗した時に旧経路へ自動フォールバックする実装は禁止。理由:
  - バグを隠す（旧経路で「動いて見える」ので不具合に気付かない。実例: `2026-04-22` に file watch の polling fallback が 11 project ぶん永遠に走り続けて入力ラグの根本原因になっていたが、画面が動いてたので長く検出できなかった）
  - UX 不整合（同じ機能でも条件次第で経路が変わる → デバッグ不能）
  - 主経路の必須化が見えなくなる（fallback あるから「いざとなれば」の安心感で eager 初期化を怠りがち）
  - 主経路が前提条件を満たせない場合は **明示的にエラー表示 / 機能無効化** する。サイレントに別経路に切り替えない
- **ロールバック用の feature flag をコードに残さない**: 「不具合があれば旧経路に戻せる」目的の flag / 旧コード共存は禁止。ロールバックは git ベース (branch / revert) で行う。理由:
  - コードに死に体の旧経路が残り続け、メンテ対象が膨らむ
  - 「優しい fallback」と地続きで、いずれサイレント分岐に堕落する
  - flag 切替の検証コストが二経路ぶん発生する
  - 大きめの切替時は (1) feature branch を切る (2) 移行 commit を載せる (3) 不具合あれば `git checkout` / `git revert` で戻す ─ のフローで対処する
